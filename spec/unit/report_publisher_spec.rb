...
describe ReportPublisher do
  let(:bindery_opts) { {email:"archivist@example.com", identity:"my_identity", pool:"the_pool", model_id:45} }
  ...
  describe "publish_reports" do
    it "" do
      ...
      expect(Cocupu).to receive(:start).with("archivist@example.com", "my_identity", 80)
      ...
      subject.publish_reports(paths_to_reports, bindery_opts)
    end
  end
